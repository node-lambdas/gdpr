function getConsent(color, bgColor) {
  return `<div id="gdpr_dialog" role="dialog" aria-live="polite" aria-label="cookieconsent"
...
onclick="localStorage.__gdpr_dismiss=1;document.getElementById('gdpr_dialog').remove()">Got it!</button></div>
</div>
<script>
localStorage.__gdpr_dismiss || document.getElementById('gdpr_dialog').style.display='block'
</script>`;
}
export default {
  version: 2,
